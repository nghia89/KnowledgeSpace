...
export class RolesService extends BaseService {
    ...
    httpPost(entity: any, path: string) {
        return this.http.post(`${environment.apiUrl}${path}`, JSON.stringify(entity), { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }

    httpPut(id: string, entity: any, path: string) {
        return this.http.put(`${environment.apiUrl}${path}${id}`, JSON.stringify(entity), { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }

    add(entity: Role) {
        return this.httpPost(entity, '/api/roles');
    ...
    update(id: string, entity: Role) {
        return this.httpPut(id, entity, '/api/roles/');
